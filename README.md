# Stop-Bad-Habits

(Indonesia)
StopBadHabits adalah platform berbasis web yang dirancang untuk membantu pengguna menghentikan kebiasaan buruk secara konsisten. Fokus utama aplikasi ini adalah pelacakan kemajuan harian dan pemberian motivasi berkelanjutan.

V1:
- Desain minimalis dengan skema warna biru cerah untuk kenyamanan visual.
- Efek transisi halus saat halaman di-scroll menggunakan library Animate On Scroll.
- Sistem catatan dengan fitur CRUD (Create, Read, Update, Delete)
- Fitur chart yang memudahkan pengguna melacak keberhasilan dan kegagalan, dan bisa juga sebagai sumber motivasi.
- Fitur kalender absen untuk mendokumentasikan keberhasilan dan kegagalan harian.

## Lisensi
Repo ini dilindungi oleh lisensi custom. Hanya boleh digunakan sebagai referensi, tidak boleh dipakai atau dimodifikasi.

(English)
StopBadHabits is a web-based platform designed to help users consistently break bad habits. The primary focus of this application is tracking daily progress and providing continuous motivation.

V1:
- Minimalist design with a bright blue color scheme for visual comfort.
- Smooth transition effects when scrolling pages using the Animate On Scroll library.
- Note-taking system with CRUD (Create, Read, Update, Delete) features.
- Chart feature that makes it easy for users to track successes and failures, and can also serve as a source of motivation.
- Absence calendar feature to document daily successes and failures.

Translated with DeepL.com (free version)

## License
This repository is protected by a custom license. It may only be used as a reference and may not be used or modified.
